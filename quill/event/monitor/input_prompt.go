...
type PromptReader interface {
	GetPromptResponse(ctx context.Context) (string, error)
}

type PromptWriter interface {
	IsSensitive() bool
	PromptMessage() string
	SetPromptResponse(string) error
...
func (p *Prompter) SetPromptResponse(value string) error {
	if p.value != nil {
...
func (p *Prompter) GetPromptResponse(ctx context.Context) (string, error) {
	if p.value != nil {
		return *p.value, nil
	}
	select {
