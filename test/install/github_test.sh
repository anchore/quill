...
test_extract_json_value() {
  fixture=./test-fixtures/github-api-quill-v0.2.0-release.json
  content=$(cat ${fixture})

  actual=$(extract_json_value "${content}" "tag_name")
  assertEquals "v0.2.0" "${actual}" "unable to find tag_name"
...
test_github_release_tag() {
  ...
  assertEquals "v0.2.0" "${actual}" "unable to find release tag"
}
...
test_download_github_release_checksums() {
  ...
  tag=v0.2.0
  github_download="https://github.com/anchore/quill/releases/download/${tag}"
  name=${PROJECT_NAME}
  version=$(tag_to_version "${tag}")

  actual_filepath=$(download_github_release_checksums "${github_download}" "${name}" "${version}" "${tmpdir}")
  assertFilesEqual \
    "./test-fixtures/quill_0.2.0_checksums.txt" \
    "${actual_filepath}" \
